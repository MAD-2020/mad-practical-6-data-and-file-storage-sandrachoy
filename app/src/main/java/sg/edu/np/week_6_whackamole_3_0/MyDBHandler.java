...
public class MyDBHandler extends SQLiteOpenHelper {
    ...
    public static String DATABASE_NAME = "WhackAMoleDB.db";
    ...
    public void addUser(UserData userData)
    {
        ...
        for (int x = 0; x < userData.getLevels().size(); x++) {
            ContentValues values = new ContentValues();
            values.put(COLUMN_USERNAME, userData.getMyUserName());
            values.put(COLUMN_PASSWORD, userData.getMyPassword());
            values.put(COLUMN_LEVEL, userData.getLevels().get(x));
            values.put(COLUMN_SCORE, userData.getScores().get(x));
            Log.v(TAG, FILENAME + ": Adding data for Database: " + values.toString());
